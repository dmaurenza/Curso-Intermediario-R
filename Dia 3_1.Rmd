...
As funções loop são usadas para aplicar a mesma tarefa para um dado conjunto de dados de acordo com os níveis de um fator. Existem diversas funções loop, mas nós vamos focar na função loop "for",
...
Nesta prática, vamos pedir que o R print (imprimir) o nome de cada aluno da planilha alunos
...
1. A sequência de Fibonacci é uma sequência famosa na matemática. Os dois primeiros números da sequência são [1, 1]. Os números subsequentes são compostos pela soma dos dois números anteriores. Assim, o terceiro número da sequência de Fibonacci é 1 + 1 = 2, o quarto é 1 + 2 = 3, e assim por diante.
...
A sequencia lógica para usarmos a função loop "for" em dataframes é similar à que usamos em vetores, porém, utilizando a indexação de dataframes .
