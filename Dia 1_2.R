...
# Assim, podemos extrair sub-conjuntos específicos de dados de acordo com as ‘coordenadas’ dos dados que queremos, assim como em um jogo de batalha naval.
...
# Antes, Vamos aprender uma nova função para criar um vetor.
# Função seq
...
# Agora sim, vamos aprender como fazer uma indexação. # Mas o que é a indexação? é a forma de extrair o valor atravéz da sua posição

# Por exemplo, qual é o valor na posição (index) 16 do vetor1?
...
# Lembram da função seq? Podemos utiliza-la nas indexações

vetor2[16:21]

# Da mesma forma como podemos selecionar os elementos que queremos, podemos selecionar aqueles elementos que não queremos!
...
# Neste caso, a indexação está usando um vetor de um ou mais elementos, que marcam as posição dos elementos a serem retidos.
...
# também podemos indexar caracteres
letters
...
top_5_letters <- c("a", "b", "c", "d", "e")
letters[1:5] <- top_5_letters
letters
...
# criando vetor de letras A e B. A função rep repete os valores indicados, e possui um argumento chamado times e quantifica quantas vezes o valor indicado deve ser repetido
letras <- rep(c("A", "B"), times = 17)
letras

# juntando os tres vetores em um data.frame
dados <- data.frame(vetor1, vetor2, letras)
dados

# para indexar um data.frame, utilizamos os colchetes separados por uma virgula. A posição antes da virgula expressa a posição da linha. A posição após a virgula expressa a posição da coluna
...
# Desafio = adicione uma quarta coluna chamada vetor3 ao data.frame "dados" repetindo a letra "C" e "D"
