...
1. [Leitura dos dados](#anchor1)
2. [Funções úteis](#anchor2)
3. [Exportando dados](#anchor3)


## Leitura dos dados {#anchor1}
...
Nesta aula vamos aprender como importar dados externos, fazer algumas manipulações e exportar (armazenar) os resultados. 
...
Vamos importar os dados da planilha alunos. Mas antes, vamos "limpar"
o ambiente que estamos trabalhando. Assim, não teremos nenhum objeto da aula anterior.
...
alunos_csv2
```
Arquivos .csv também podem ser importados através do pacote "tidyverse". Vamos ver
```{r}
...
<center><img src="img/CI.png" height="250px"></img></center>  
...
Função "sub()". 
A função "sub()" permite substituir o valor de um vetor, ou de um vetor dentro de um dataframe, ou lista
...
Função "round()"
Usamos a função "round" para arredondar valores numéricos. Para exemplificar, vamos criar um vetor com 100 valores com uma distribuição  normal, sendo a média = 3000 e o desvio padrão = 2000
```{r}
...
"Sample" significa "amostra" em inglês. Assim, podemos depreender que a função faz uma amostragem de um vetor
```{r}
...
"Unique" seleciona os elementos únicos de um vetor
```{r}
...
"Paste", em inglês, significa colar. Portanto, depreende-se que a função permite colar diversos objetos em apenas 1 objeto. Esta função é muito utilizada para concatenar caracteres. 
```{r}
nome_1 <- "Paulo"
nome_2 <- "Maria"
nome_3 <- "João"
